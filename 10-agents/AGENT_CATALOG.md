...
| Workflow | Purpose |
|---|---|
| `repo-agent-loop` | Routes a repo task, curates the source layer, generates an enrichment plan, packages the decision, runs repository checks, and returns a consolidated result |
...
These are natural next additions once the first five are stable:
