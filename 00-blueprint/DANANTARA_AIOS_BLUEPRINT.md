...
`danantara-master-orchestrator` -> `source-layer-curator` -> `source-layer-enrichment` -> `repo-audit-gate`

First workflow:
`repo-agent-loop` stitches the four agents together with the repo audit and
